...
/* Stock: producido - GREATEST(reservas, entregas) */
$sql = "SELECT
            t.nombre AS tipo,
            ...
            (
              ...
              - GREATEST(
                  ...
                  IFNULL((SELECT SUM(e.cantidad)
                          FROM entregas_stock e
                          JOIN reservas_productos r2 ON r2.id = e.id_reserva
                          WHERE r2.id_variedad = v.id AND r2.estado >= 0), 0)
                )
...
while ($row = mysqli_fetch_assoc($r)) {
  ...
  switch ($tipo) {
    ...
    case 'ÁRBOLES':
    case 'ARBOLES': $arboles[]=$row; break;
...
function render_catalogo(array $ps): void {
  foreach ($ps as $p) {
    $pm = number_format(((float)$p['precio'])*1.19,0,',','.');
    $img = !empty($p['imagen']) ? "https://control.roelplant.cl/uploads/variedades/".htmlspecialchars($p['imagen'],ENT_QUOTES,'UTF-8') : "https://via.placeholder.com/600x400?text=Imagen+pendiente";
    ...
    <div class="producto" role="button" tabindex="0"
         aria-label="Ver detalle <?= htmlspecialchars($p['variedad'],ENT_QUOTES,'UTF-8'); ?>"
         data-nombre="<?= htmlspecialchars($p['variedad'],ENT_QUOTES,'UTF-8'); ?>"
         data-ref="<?= htmlspecialchars($p['referencia'],ENT_QUOTES,'UTF-8'); ?>"
         data-stock="<?= (int)$p['disponible_para_reservar']; ?>"
      ...
      <div class="img-wrap">
        <img src="<?= $img ?>" alt="Imagen de producto" loading="lazy" decoding="async">
      </div>
      <h3 class="variedad"><?= htmlspecialchars($p['variedad'],ENT_QUOTES,'UTF-8'); ?></h3>
      <p><strong>Referencia:</strong> <?= htmlspecialchars($p['referencia'],ENT_QUOTES,'UTF-8'); ?></p>
      <p><strong>Stock Disponible:</strong> <?= (int)$p['disponible_para_reservar']; ?></p>
...
<style>
:root{--brand:#27ae60;--ink:#2c3e50;--card:#fff;--muted:#555;--menu:#1f7a4e}
...
/* Grid y cards */
.catalogo{display:grid;grid-template-columns:repeat(auto-fit,minmax(260px,1fr));gap:20px;max-width:1200px;margin:0 auto}
.producto{background:var(--card);border-radius:14px;box-shadow:0 8px 20px rgba(0,0,0,.08);padding:16px;text-align:center;transition:transform .18s ease, box-shadow .18s ease;border-top:5px solid var(--brand);cursor:pointer}
...
.variedad{text-transform:uppercase;font-weight:800;color:var(--brand);margin:8px 0 10px;font-size:1.12em;line-height:1.2}
.producto p{color:var(--muted);margin:5px 0}
.producto strong{color:#000}
.acciones{display:flex;gap:10px;justify-content:center;margin-top:12px}
.btn-detalle,.btn-reservar{background:var(--brand);color:#fff;padding:12px 14px;border:0;border-radius:10px;cursor:pointer;min-width:120px}
.btn-detalle{background:#2d6fb6}
.btn-detalle:hover{background:#1f6690}
.btn-reservar:hover{background:#1e8a4c}
.descripcion-snippet{font-size:.95em;color:#3e3e3e;min-height:42px}
...
.modal h3{margin:0 0 6px;color:var(--ink)}.modal p{margin:6px 0;color:#333}
...
/* Burbujas */
.whatsapp-bubble{position:fixed;bottom:20px;left:20px;background:#25D366;border-radius:50%;width:56px;height:56px;display:flex;align-items:center;justify-content:center;cursor:pointer;z-index:9000;box-shadow:0 4px 8px rgba(0,0,0,.3)}
.whatsapp-bubble img{width:32px;height:32px}
.ig-bubble{position:fixed;bottom:20px;right:20px;background:linear-gradient(45deg,#f58529,#dd2a7b,#8134af,#515bd4);border-radius:50%;width:56px;height:56px;display:flex;align-items:center;justify-content:center;cursor:pointer;z-index:9000;box-shadow:0 4px 8px rgba(0,0,0,.3)}
...
.tech-menu a{display:block;padding:10px 12px;text-decoration:none;color:#334155;border-radius:10px;line-height:1.25}
...
@media (max-width:980px){
  ...
  .whatsapp-bubble{bottom:84px} /* evita tapar CTAs al abrir menú */
  /* Botón Filtros */
...
<?php foreach($secciones as $id=>$sec){ if(count($sec[1])===0) continue; ?>
  ...
  <div class="catalogo"><?php render_catalogo($sec[1]); ?></div>
<?php } ?>

<button class="btn-detalle" style="display:block;margin:40px auto;border-radius:12px" onclick="window.location.href='catalogo_tabla.php'">Ver listado</button>
...
<script>
/* Mostrar botón Filtros solo en móvil */
const mq = window.matchMedia('(max-width:980px)');
const toggleBtn = document.getElementById('menuToggle');
function updateToggle(){ toggleBtn.style.display = mq.matches ? 'flex' : 'none'; }
mq.addEventListener ? mq.addEventListener('change', updateToggle) : mq.addListener(updateToggle);
updateToggle();
...
document.querySelectorAll('.tech-menu a[href^="#"]').forEach(a=>{
  a.addEventListener('click',e=>{
    ...
    if(t){ e.preventDefault(); if(menu.classList.contains('open')) menu.classList.remove('open');
