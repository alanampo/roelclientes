...
$orders = [];
$st2 = mysqli_prepare($db, "SELECT id, status, total_clp, created_at FROM orders WHERE customer_id=? ORDER BY id DESC LIMIT 10");
mysqli_stmt_bind_param($st2, 'i', $id);
mysqli_stmt_execute($st2);
$res2 = mysqli_stmt_get_result($st2);
while ($res2 && ($r = mysqli_fetch_assoc($res2))) $orders[] = $r;
...
<div class="grid grid-2">
  ...
  <aside class="card">
    ...
    <div class="card-b">
      <div class="row">
        <a class="btn" href="orders.php?q=<?= h(urlencode((string)$c['email'])) ?>">Buscar pedidos stock</a>
    ...
    <div class="card-b">
      <table class="table">
        ...
        <tbody>
          ...
          <?php endforeach; ?>
          <?php if (!count($orders)): ?><tr><td colspan="4" class="muted">Sin pedidos.</td></tr><?php endif; ?>
        </tbody>
      </table>
    </div>
  </section>
