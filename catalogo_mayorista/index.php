...
/* -------- Query: catálogo mayorista (solo lectura a tu BD stock) --------
   FIX:
   - tipo_planta se obtiene con agregación condicional (MAX + CASE)
   - attrs_activos agrupa TODOS los atributos distintos a TIPO DE PLANTA (para card + popup)
-------- */
$sql = "
SELECT
  sv.id_variedad,
...
if(!$r){
  die("Error SQL catálogo mayorista: ".mysqli_error($link));
}
...
/* -------- Constantes URL -------- */
$catalogoBase = 'https://clientes.roelplant.cl/catalogo_mayorista/';
...
$ogDesc  = 'Plantines mayoristas con envío a todo Chile. Regla: mínimo 50 por especie y total ≥ 200 unidades.';
$ogUrl   = $catalogoBase;
...
function render_catalogo(array $ps,string $catalogoBase,string $priceValidUntil):void{
  foreach($ps as $p){
    ...
    $precioVisibleTxt = $pm ? ('$'.$pm.' (Mayorista, imp. incl.)') : 'Precio no disponible';
    ...
    <div class="producto" role="button" tabindex="0"
      ...
      <div class="contenido">
        ...
        <p><strong>Precio mayorista:</strong> <?= $pm ? ('$'.$pm.' Imp. incl.') : 'No disponible' ?></p>

        <p class="muted" style="font-size:12px;margin-top:6px">Regla mayorista: mínimo 50 por especie y total ≥ 200 unidades.</p>
      ...
      <div class="acciones">
        ...
        <button class="btn-reservar" onclick="event.stopPropagation(); addToCartFromCard(this.closest('.producto'), 50);">Agregar 50</button>
      </div>
...
<meta name="description" content="Catálogo mayorista Roelplant: mínimo 50 por especie y total ≥ 200 unidades. Interior, exterior, cubresuelos y más. Envíos a todo Chile.">
<meta name="author" content="Roelplant">
<meta name="keywords" content="plantines, catalogo mayorista, vivero, plantinera, plantas interior, plantas exterior, cubresuelos, quillota, chile">
...
<script type="application/ld+json">
{
  ...
  "@type":"WebSite",
  "name":"Roelplant",
  "url":"https://clientes.roelplant.cl/catalogo_mayorista/",
  "potentialAction":{
    "@type":"SearchAction",
    "target":"https://clientes.roelplant.cl/catalogo_mayorista/?s={search_term_string}",
...
<script type="application/ld+json">
{
  "@context":"https://schema.org",
  "@type":"ItemList",
  "name":"Catálogo Mayorista Roelplant",
...
<div class="topbar" role="banner">
  <div class="brand">
    ...
    <strong>Roelplant · Catálogo mayorista</strong>
  </div>
...
<div style="max-width: 900px; margin: 24px auto 40px; padding: 20px; background-color: #fffbe6; border: 2px dashed #f39c12; border-radius: 10px; color: #555;">
  ...
  <p><strong>📦 Catálogo Mayorista:</strong> A partir de 200 plantines accedes al precio mayorista. Envíos a todo Chile con Starken y otros operadores logísticos. Todos los envíos deben ser previamente cancelados.
    ...
    <div>
      ...
      <div class="acciones-modal">
        <div class="qty-ctl">
          ...
          <input class="inp" id="mQty" type="number" min="50" value="50">
          <button type="button" onclick="chgModalQty(1)">+</button>
        </div>
      ...
      <div class="notice">Regla mayorista: mínimo 50 por especie y total ≥ 200 unidades. Para agregar al carrito debes iniciar sesión con tu correo.</div>
    </div>
  </div>
</div>
...
<script>
(function(){
  const orig = window.openProductoModal;
  if(typeof orig !== 'function') return;

  const hasV = (u) => /[?&]v=\d+/.test(String(u||''));
  const bust = (u) => {
    u = String(u||'').trim();
    if(!u) return u;
    if(hasV(u)) return u;
  ...
  function buildAttrsBox(attrsRaw){
    const raw = String(attrsRaw||'').trim();
    if(!raw) return null;
  ...
  window.openProductoModal = function(card){
    ...
    try{
      const el = card && card.dataset ? card : null;
      if(!el) return;

      // 1) Imagen (evita "pendiente" por cache)
      const mImg = document.getElementById('mImagen');
      const img = el.dataset.imagen || el.getAttribute('data-imagen') || '';
      if(mImg && img){
        mImg.src = bust(img);
      }

      // 2) Atributos en popup (inserta arriba de descripción)
      const mDesc = document.getElementById('mDesc');
      if(mDesc){
        const prev = mDesc.querySelector('.attrs');
        if(prev) prev.remove();

        const attrs = el.dataset.attrs || el.getAttribute('data-attrs') || '';
        const box = buildAttrsBox(attrs);
        if(box) mDesc.prepend(box);
      }
    }catch(e){}
  };
})();
...
(function(){
  ...
  // Prefill desde ?s= (SearchAction JSON-LD)
