...
$kpi = [
  ...
  'orders_may'   => (int)($db->query("SELECT COUNT(*) c FROM orders_may")->fetch_assoc()['c'] ?? 0),
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($id > 0 && bo_status_is_allowed($newStatus)) {
    if ($action === 'order_status') {
      $st = mysqli_prepare($db, "UPDATE orders_may SET status=? WHERE id=?");
...
if ($tab === 'pedidos') {
  ...
  mysqli_stmt_execute($st);
  $rs = mysqli_stmt_get_result($st);
  while ($r = $rs->fetch_assoc()) $orders_may[] = $r;
  mysqli_stmt_close($st);
}
        ...
        <div class="card-b">
          <?php if ($tab === 'kpis'): ?>
            <div class="kpis">
              ...
              <div class="kpi"><div class="t">Pedidos stock</div><div class="v"><?=number_format($kpi['orders_may'],0,',','.')?></div></div>
          ...
          <?php elseif ($tab === 'pedidos'): ?>
            ...
            <?php if ($viewOrderId > 0): ?>
              <?php
                $st = mysqli_prepare($db, "SELECT * FROM orders_may WHERE id=?");
                if ($st) {
                ...
                $items=[];
                if ($od) {
                  $st2 = mysqli_prepare($db, "SELECT nombre AS product_name, qty, unit_price_clp, line_total_clp, imagen_url, referencia, id_variedad FROM order_items_may WHERE order_id=? ORDER BY id ASC");
                  if ($st2) {
