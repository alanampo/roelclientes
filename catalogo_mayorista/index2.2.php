...
/* -------- Query: catálogo detalle (solo lectura a tu BD stock) --------
   FIX: antes ANY_VALUE(av.valor) podía "tomar" un valor de otro atributo (ej: BOLSA)
   y romper la categoría tipo_planta, haciendo que el producto "desaparezca".
   Ahora:
   - tipo_planta se obtiene con agregación condicional (MAX + CASE)
   - attrs_activos agrupa TODOS los atributos distintos a TIPO DE PLANTA para mostrarlos en la card
-------- */
$sql = "
SELECT
  sv.id_variedad,
...
function render_catalogo(array $ps,string $catalogoBase,string $priceValidUntil):void{
  foreach($ps as $p){
    ...
    $precioVisibleTxt = $pm ? ('$'.$pm.' (Mayorista, imp. incl.)') : ($pd ? ('$'.$pd.' (Detalle, imp. incl.)') : 'Precio no disponible');
