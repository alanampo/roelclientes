...
-- Migración desde esquema legacy (orders_may.user_id, order_items_may.product_ref)
-- al esquema v2 (orders_may.order_code, order_items_may.referencia, etc.)
--
...
-- Si tienes datos importantes en orders_may/order_items_may, se conservan aquí:
RENAME TABLE
  orders_may TO orders_legacy,
  order_items_may TO order_items_legacy;

-- Crear tablas v2
-- (mismo contenido que catalogo_detalle/sql/03_orders.sql)

CREATE TABLE IF NOT EXISTS orders_may (
...
CREATE TABLE IF NOT EXISTS order_items_may (
  id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  ...
  CONSTRAINT fk_order_items_order
    FOREIGN KEY (order_id) REFERENCES orders_may(id)
    ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
