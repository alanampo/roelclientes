...
function openProductoModal(card){
  ...
  // Catálogo mayorista: siempre mostrar precio mayorista.
  ...
  pmEl.textContent=pm?('Precio mayorista: $'+pm+' Imp. incl.'):'Precio no disponible';
  document.getElementById('mDesc').textContent=card.dataset.descripcion||'Sin descripción';
  // Regla mayorista: mínimo 50 por especie.
  document.getElementById('mQty').value='50';
...
async function addToCartCurrent(){
  ...
  let qty = parseInt(document.getElementById('mQty').value||'50',10) || 50;
  if(qty < 50) qty = 50;
...
async function loadCart(){
  ...
  items.forEach(it=>{
    ...
    row.innerHTML = `
      ...
      <div style="display:flex;flex-direction:column;gap:8px;align-items:flex-end">
        <div class="qty-ctl">
          ...
          <input class="inp" type="number" min="50" value="${it.qty||50}">
    ...
    btnMinus.addEventListener('click',()=>{
      let v=parseInt(inp.value||'50',10); v=isNaN(v)?50:v;
      v=Math.max(50,v-1);
      inp.value=String(v);
      applyQty(v);
    });
  ...
  const hasInvalidLine = items.some(it => Number(it.qty||0) > 0 && Number(it.qty||0) < 50);
  const rulesOk = !hasInvalidLine && qtyTotal >= 200;

  if(!rulesOk){
    const parts=[];
    if(hasInvalidLine) parts.push('Cada especie debe tener mínimo 50 unidades.');
    if(qtyTotal < 200) parts.push(`Total actual ${qtyTotal}. Debe ser ≥ 200 unidades.`);
    setErr('cartErr', parts.join(' '));
  }

  if(btn){
    btn.disabled = (j.cart.item_count||0)===0 || !rulesOk;
