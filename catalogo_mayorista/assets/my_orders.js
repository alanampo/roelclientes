(function(){
  ...
  async function init(){
    ...
    $('ordersMeta').textContent = 'Pedidos encontrados: '+(list.orders_may?.length||0);
    const wrap = $('ordersList');
    wrap.innerHTML = '';
    if(!list.orders_may || list.orders_may.length===0){
      $('ordersEmpty').style.display='block';
      return;
    }
    list.orders_may.forEach(o=> wrap.appendChild(orderCard(o)));
  }
