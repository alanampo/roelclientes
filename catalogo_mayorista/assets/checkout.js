...
const notesEl = $('notes');
  ...
  async function createOrder() {
    ...
    // Validación mayorista: mínimo 50 por especie y total >= 200 unidades.
    try {
      const cj = await fetchJson('api/cart/get.php', { method: 'GET' });
      const items = (cj.cart && cj.cart.items) ? cj.cart.items : [];
      const qtyTotal = items.reduce((acc,it)=>acc+Number(it.qty||0),0);
      const invalid = items.filter(it => Number(it.qty||0) > 0 && Number(it.qty||0) < 50);
      if (invalid.length) {
        const bad = invalid.map(it => `${(it.nombre||'')} (${(it.referencia||'')}) = ${Number(it.qty||0)}`).join(' · ');
        showAlert('Mínimo 50 unidades por especie. Revisa: ' + bad, 'danger');
        return;
      }
      if (qtyTotal < 200) {
        showAlert(`El pedido mayorista debe tener total ≥ 200 unidades. Total actual: ${qtyTotal}`, 'danger');
        return;
      }
    } catch (e) {
      // Si la validación falla por conexión, dejar que el servidor valide.
    }
