...
$hasV2 = table_has_column($db,'orders_may','order_code') && table_has_column($db,'orders_may','customer_id');
$itemsV2 = table_has_column($db,'order_items_may','referencia'); // v2
if ($hasV2) {
  $sql = "SELECT id, order_code, status, subtotal_clp, shipping_label, shipping_cost_clp, total_clp, notes, created_at
          FROM orders_may WHERE id=? AND customer_id=? LIMIT 1";
  $st = $db->prepare($sql);
  ...
  $items = [];
  if ($itemsV2) {
    $st = $db->prepare("SELECT referencia, nombre, imagen_url, unit_price_clp, qty, line_total_clp FROM order_items_may WHERE order_id=? ORDER BY id ASC");
  ...
  $sql = "SELECT id, status, shipping_label, shipping_amount, subtotal_amount, total_amount, notes, created_at
          FROM orders_may WHERE id=? AND user_id=? LIMIT 1";
  $st = $db->prepare($sql);
  if (!$st) json_out(['ok'=>false,'error'=>'No se pudo preparar consulta'],500);
  $st->bind_param('ii',$id,$cid);
  $st->execute();
  ...
  $items = [];
  $st = $db->prepare("SELECT product_ref, product_name, image_url, unit_price, qty, line_total FROM order_items_may WHERE order_id=? ORDER BY id ASC");
  if (!$st) json_out(['ok'=>false,'error'=>'No se pudo preparar items'],500);
  $st->bind_param('i',$id);
  $st->execute();
  $res = $st->get_result();
