...
function start_session(): void {
  ...
  if (session_status() !== PHP_SESSION_ACTIVE) {
    @session_name('ROELCARTMAYSESSID');
  }
...
function cart_get_or_create(mysqli $db, int $customerId): int {
  ...
  $q = "SELECT id FROM carts_may WHERE customer_id=? AND status='open' LIMIT 1";
  $st = $db->prepare($q);
  ...
  // 2) crear
  $q2 = "INSERT INTO carts_may (customer_id, status) VALUES (?, 'open')";
  $st2 = $db->prepare($q2);
