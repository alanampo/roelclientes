...
if ($qty === 0) {
  $del = $db->prepare("DELETE FROM cart_items_may WHERE id=? AND cart_id=?");
  $del->bind_param('ii', $itemId, $cartId);
  $del->execute();
  json_out(['ok'=>true,'cart'=>cart_snapshot($db, $cartId)]);
}

// Reglas mayoristas: si no se elimina, debe quedar en mínimo 50.
if ($qty > 0 && $qty < 50) bad_request('Mínimo 50 unidades por especie.');

$up = $db->prepare("UPDATE cart_items_may SET qty=? WHERE id=? AND cart_id=?");
