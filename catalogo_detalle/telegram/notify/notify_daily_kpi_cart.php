...
$k['orders_today'] = (int)$db->query("SELECT COUNT(*) FROM orders WHERE created_at BETWEEN '{$start}' AND '{$end}'")->fetchColumn();
$k['revenue_today'] = (int)$db->query("SELECT COALESCE(SUM(total_clp),0) FROM orders WHERE created_at BETWEEN '{$start}' AND '{$end}'")->fetchColumn();
$k['open_carts'] = (int)$db->query("SELECT COUNT(*) FROM carts WHERE status='open'")->fetchColumn();
$k['abandoned_carts'] = (int)$db->query("SELECT COUNT(*) FROM carts WHERE status='abandoned'")->fetchColumn();
